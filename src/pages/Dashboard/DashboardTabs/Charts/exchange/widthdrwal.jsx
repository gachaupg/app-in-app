...
const WidthGraph = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const depositOrders = transactions.filter(item => item.transaction_type === 'withdrawal');
    ...
    useEffect(() => {
        if (chartRef.current && data.length > 0) {
            ...
            const gradient = ctx.createLinearGradient(0, 1, 0, ctx.canvas.height);
            gradient.addColorStop(0, 'red');  
            gradient.addColorStop(1, '#5B0E02FF');  
